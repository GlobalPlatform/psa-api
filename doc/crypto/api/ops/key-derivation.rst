...
.. function:: psa_key_derivation_setup
    ...
    If `psa_key_derivation_setup()` is called with an operation object that is not inactive, it returns :code:`PSA_ERROR_BAD_STATE` and the operation enters an error state.

    If `psa_key_derivation_setup()` returns an error when called with an inactive operation object, the operation object is unchanged.
